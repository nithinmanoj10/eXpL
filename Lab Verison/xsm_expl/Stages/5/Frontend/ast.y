...
%type <node> start Slist Stmt inputStmt outputStmt assignStmt ifStmt whileStmt doWhileStmt breakStmt continueStmt breakPointStmt ID expr NUM STRING 
...
GID			:	ID						{ GSTInstall($1->nodeName, getDeclarationType(), 1, NULL, -1); }
			|	ID '[' NUM ']'			{ 
											if ($3->intConstVal < 1) {
												printf("\nArray Declaration expects valid size\n");
												exit(1);
											}
											GSTInstall($1->nodeName, getDeclarationType(), $3->intConstVal, NULL, -1); 
										}
			/* |	ID '(' ParamList ')'	{ printParamList(); } */
			|	MUL ID					{
											if (getDeclarationType() == TYPE_INT)
												GSTInstall($2->nodeName, TYPE_INT_PTR, 1, NULL, -1);

											if (getDeclarationType() == TYPE_STR)
												GSTInstall($2->nodeName, TYPE_STR_PTR, 1, NULL, -1);
										}
			;
