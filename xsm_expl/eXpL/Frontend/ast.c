...
struct ASTNode *TreeCreate(struct TypeTable *typeTablePtr, int nodeType, char *nodeName, int intConstVal, char *strConstVal, struct ASTNode *left, struct ASTNode *middle, struct ASTNode *right, struct ASTNode *loopStmts)
{
    ...
    newASTNode->loopStmts = loopStmts;
...
void printASTTable(struct ASTNode *root, int sno)
{
    ...
    printf("SNo         NodeType       DataType      ClassType            NodeName  intConstVal          strConstVal  Left  Middle  Right  loopStmts\n");
    printf("────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────\n\n");
    printAST(root, sno);
}
...
int getVariableAddress(FILE *filePtr, struct ASTNode *root)
{
    ...
    if (varLSTEntry == NULL)
    {
        ...
        // for 2-D array variable
        if (root->left != NULL && root->middle != NULL)
        {
            int offsetReg = getReg();

            fprintf(filePtr, "MOV R%d, R%d\n", offsetReg, evalExprTree(filePtr, root->left));
            freeReg(); // resultReg from evalExprTree()

            fprintf(filePtr, "MUL R%d, %d\n", offsetReg, root->GSTEntry->columnSize);

            fprintf(filePtr, "ADD R%d, R%d\n", offsetReg, evalExprTree(filePtr, root->middle));
            freeReg(); // resultReg from evalExprTree()
...
int evalExprTree(FILE *filePtr, struct ASTNode *root)
{
    ...
    if (root->nodeType == FUNC_NODE || root->nodeType == MEM_FUNC_NODE || root->nodeType == SELF_FUNC_NODE)
    {
        ...
        struct ASTNode *argListHead = (root->right == NULL) ? (root->argListHead) : (root->right->argListHead);
...
int verifyFunctionArguments(char *funcName, struct ASTNode *argumentList)
{
    ...
    while (funcParamList != NULL)
    {
        ...
        {
            printf("\n%s() expects data type %s for argument %d\n", funcName, (funcParamList->typeTablePtr->typeName) ? ("int") : ("str"), argCount);
            exit(1);
        }
